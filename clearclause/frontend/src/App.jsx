...
function App() {
  const [document, setDocument] = useState(null); // { text, metadata }
  const [sessionData, setSessionData] = useState({
    briefing: null,
    riskFlags: [],
    qna: []
  });
  const [currentView, setCurrentView] = useState('upload'); // upload, concerns, dashboard
  const [isLoading, setIsLoading] = useState(false);

  const handleUploadComplete = (data) => {
    setDocument({ text: data.extractedText, metadata: data.metadata });
  ...
  const handleConcernsSelected = async (topics) => {
    ...
    try {
      // Trigger Risk Scan
      const riskRes = await fetch('/api/scan-risks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ documentText: document.text })
      });
      ...
      const briefRes = await fetch('/api/brief', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ documentText: document.text, topics })
      });
      ...
      <main className="flex-1 flex flex-col relative">
        {currentView === 'upload' && (
          <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-4xl mx-auto w-full">
            <div className="text-center mb-10">
              <h2 className="text-4xl font-extrabold mb-4 text-brand-navy">You signed it. <span className="text-brand-green">Now understand it.</span></h2>
              <p className="text-slate-600 text-lg max-w-2xl mx-auto">Upload any legal document to get plain-English explanations, risk flags, and grounded answers directly from the text.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
              <div className="flex flex-col items-center text-center p-4">
                ...
                <h3 className="font-semibold text-lg mb-2">Grounded Answers</h3>
                <p className="text-slate-500 text-sm">Every answer cites the exact clause in your document. No hallucinated legal advice.</p>
              </div>
              <div className="flex flex-col items-center text-center p-4">
                <FileText className="w-10 h-10 text-brand-amber mb-3" />
                <h3 className="font-semibold text-lg mb-2">Risk Detection</h3>
                <p className="text-slate-500 text-sm">Automatically flags unusual terms, missing protections, or aggressive language.</p>
              </div>
              <div className="flex flex-col items-center text-center p-4">
                <Scale className="w-10 h-10 text-brand-navy mb-3" />
                <h3 className="font-semibold text-lg mb-2">Privacy First</h3>
                <p className="text-slate-500 text-sm">Your documents are processed in memory and never stored permanently.</p>
              </div>
            </div>
          </div>
        )}
        ...
        {currentView === 'concerns' && (
          <div className="flex-1 p-8">
            <ConcernSelector onProceed={handleConcernsSelected} />
          </div>
        )}
